...
public class Rq {
	...
	public Rq(MemberService memberService, NoticeService noticeService, MessageSource messageSource, LocaleResolver localeResolver, HttpServletRequest req, HttpServletResponse resp, HttpSession session) {
		...
		if (authentication.getPrincipal() instanceof Member) {
			this.member = (Member) authentication.getPrincipal();
		} else {
			this.member = null;
		}
	}
	...
	public Member getMember() {
		...
		// 데이터가 없는지 체크
		if (member == null) {
			member = memberService.findMemberByUsername(member.getUsername());
		}
